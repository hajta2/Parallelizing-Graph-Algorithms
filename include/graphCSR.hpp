...
void sve_2(const int NOVertices, const int *csrRowPtr,
           const int *csrColInd, const float *csrVal,
           const float *weights, float *flow) {
    #pragma omp parallel for
    for (int i = 0; i < NOVertices; ++i) {
...
void sve_3(const int NOVertices, const int *csrRowPtr,
           const int *csrColInd, const float *csrVal,
           const float *weights, float *flow) {
    #pragma omp parallel for
    for (int i = 0; i < NOVertices; ++i) {
...
//                 int indlo = start + j;
//                 int indhi = start1 + j;
//                 pgHalf = svwhilelt_b32(j, start1);
//                 pgFull = svwhilelt_b32(j, end);
//                 svfloat32_t valuelo = svld1_f32(pgHalf, csrVal + indlo);
//                 svfloat32_t valuehi = svld1_f32(pgHalf, csrVal + indhi);
//                 svfloat32_t value = svsplice_f32(pgFull, valuelo, valuehi);

//                 svint32_t columnlo = svld1_s32(pgHalf, csrColInd + indlo);
//                 svint32_t columnhi = svld1_s32(pgHalf, csrColInd + indhi);
//                 svint32_t column = svsplice_s32(pgFull, columnlo, columnhi);

//                 svfloat32_t x_val = svld1_gather_index(pgFull, weights, column);

//                 svsum = svmla_m(pgFull, svsum, value, x_val);
//             }
...
private:
    ...
    void getWeightedFlow() override {
        ...
        if (type == NAIVE) {
            ...
            sve_2(NOVertices, csrRowPtr.data(), csrColInd.data(),
                  csrVal.data(), weights.data(), flow.data());
        } else if (type == SVE3) {
            sve_3(NOVertices, csrRowPtr.data(), csrColInd.data(),
                  csrVal.data(), weights.data(), flow.data());
        } else if (type == SVE4) {
...
//     }
    double measureARM_result() {
        auto func = [&]() {getWeightedFlowARM();};
        (void)measure(func);
        return measure(func);
    }
};
